...
window.addEventListener('load', function () {
	...
	if (mediaQuery.matches) {
		...
		menuHover.addEventListener('mouseover', function (e) {
			e.preventDefault();
			document.querySelector('.header__list>.menu-item-has-children>ul').style.maxHeight = '1000px';
	...
	//Появление поиска на пк
	if (mediaQuery.matches) {
		let searchBtn = document.querySelector('.header-search__fake');
		searchBtn.addEventListener('click', event => {
			let search = document.querySelector('.header-search form input[type=text]');
			let searchSubmit = document.querySelector('.ic-search');
